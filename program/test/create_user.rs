...
mod create_user {
	...
	fn test_transaction_is_ok() {
		...
		let (test_validator, fee_payer_info) = TestValidatorGenesis::default()
			.add_program("elumart", program_id)
			.start();
		let rpc_client = test_validator.get_rpc_client();

		let user_name: String = Default::default();
		let profile_url: String = Default::default();
		let user_record_seed_signer: Pubkey = Pubkey::new_unique();
		...
		let data = ElumartInstruction::CreateUser(crate::generated::instructions::CreateUserArgs{
			user_name,
			profile_url,
			user_record_seed_signer,
		});
		let signers = vec![
			&fee_payer_info,
        ];
